...
function buildOffense(roster,box){
  const qb=rankRoster(roster,['QB'],1)[0],rbs=rankRoster(roster,['RB','FB'],2),wrs=rankRoster(roster,['WR'],2),tes=rankRoster(roster,['TE'],1),ol=rankRoster(roster,['LT','LG','C','RG','RT','OT','T','G','OG','OL'],5);
  return `<div class="matchupOffense"><div class="matchupOffRow skills">${playerCard(wrs[0],findBoxPlayer(box,wrs[0]))}${playerCard(tes[0],findBoxPlayer(box,tes[0]))}${playerCard(wrs[1],findBoxPlayer(box,wrs[1]))}</div><div class="matchupOffRow ol">${Array.from({length:5},(_,i)=>playerCard(ol[i],findBoxPlayer(box,ol[i]),'compact')).join('')}</div><div class="matchupOffRow"><div class="matchupCenterCol">${playerCard(qb,findBoxPlayer(box,qb),'qb')}</div></div><div class="matchupOffRow backfield">${playerCard(rbs[0],findBoxPlayer(box,rbs[0]))}${playerCard(rbs[1],findBoxPlayer(box,rbs[1]))}</div></div>`;
}
function teamBoard(side,offRoster,defRoster,boxByTeam,oppBox){
  ...
  return `<section class="matchupTeamBoard"><div class="matchupTeamHead"><img class="matchupTeamLogo" src="${escM(logo)}" alt=""><div class="matchupTeamHeadText"><b>${escM(side.name||teamNameM(side.abbr))}</b><small>${escM(side.abbr)} offense vs opponent defense</small></div><div class="matchupScore"><strong>${escM(score)}</strong><span>${status==='pre'?'pregame':'score'}</span></div></div><div class="matchupField"><div class="matchupZoneLabel">Opponent defense · individual game stats</div>${buildDefense(defRoster,oppBox)}<div class="matchupLos"></div><div class="matchupZoneLabel">${escM(side.abbr)} offense · offensive personnel</div>${buildOffense(offRoster,boxByTeam)}<div class="matchupLegend">Depth-chart-first personnel approximation · personnel is not snap-specific</div></div></section>`;
}
